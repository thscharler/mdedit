...
pub fn create_mdedit_theme(name: &str) -> SalsaTheme {
    ...
    match theme.cat {
        Category::Light => {
            theme.define_style(
                Style::TEXT_BASE,
                theme.p.style_alias(Color::CONTAINER_BASE_BG),
            );
...
fn choice_tools(th: &SalsaTheme) -> ChoiceStyle {
    ChoiceStyle {
        ...
        popup_style: Some(th.p.style(Colors::Black, 3)),
...
fn statusline_hidden(th: &SalsaTheme) -> StatusLineStyle {
    ...
    for s in &mut m.styles {
        *s = mem::take(s).fg(th.p.color(Colors::Gray, 0));
    }
    m
}
...
fn menu_hidden(th: &SalsaTheme) -> MenuStyle {
    ...
    m.style = m.style.fg(th.p.color(Colors::Gray, 0));
    m
}
